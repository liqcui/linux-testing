...
- `detailed_analysis.txt` - 深度解读报告（含典型示例）

## 结果解读指南

### 自动生成深度解读

高级测试会自动生成深度解读报告 `detailed_analysis.txt`，包含：

1. **典型输出示例** - 展示各项测试的预期结果
2. **关键指标解读** - 解释每个数据点的含义
3. **异常模式识别** - 帮助识别性能问题
4. **优化建议** - 针对性的改进方案
...
# 查看解读报告
cat results/lmbench-advanced-*/detailed_analysis.txt
```
...
Memory Copy Bandwidth:
  ...
  32KB       6.2 GB/s    L2              ← 超出L1缓存（典型32KB）
   1MB       3.2 GB/s    L3              ← 接近L3缓存带宽
  64MB       2.5 GB/s    内存            ← 稳定在内存带宽
```

**关键点识别:**
- 带宽平台区: L1(~12GB/s) → L2(~6GB/s) → L3(~3GB/s) → 内存(~2.5GB/s)
- 缓存边界: 在 32KB, 512KB, 8MB 处明显下降
- 异常波动: 超过20%波动需要调查
...
Random Access Latency:
  ...
  64KB      4.5 ns      L2            ← L1→L2 明显跳跃（3x）
   1MB     18.5 ns      L3            ← L2→L3 明显跳跃（3x）
  16MB     65.3 ns      内存          ← L3→内存 明显跳跃（2.6x）
```

**关键点识别:**
- 延迟稳定区: 各级缓存内延迟应保持稳定
- 延迟跳跃点: 典型跳跃2-4倍，识别缓存边界
- Stride影响: stride越大，延迟越高（更多缓存行）
...
Process Context Switch:
  ...
  2          0B        2.3 us      ← 基准最低延迟
  8          0B        4.5 us      ← 接近CPU核心数（假设8核）
  16         0B        6.8 us      ← 超过核心数，调度竞争
  64         0B        25.3 us     ← 极高调度开销
```

**关键点识别:**
- 基准延迟: 2进程/0字节 应为 1-3us
- 最优进程数: 延迟最低点通常 ≈ CPU核心数
- 数据量影响: 每KB数据增加40-60%延迟

#### 系统调用解读

```
System Call Latency:
  getpid()          0.05 us     ← vDSO优化（最快）
  getppid()         0.15 us     ← 简单调用
  open/close        2.3 us      ← 文件系统调用
```
